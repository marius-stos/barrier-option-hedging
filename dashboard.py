...
def simulate(S0, K, B, T_days, r, sigma, n_paths, tc, seed=42):
    ...
    payoffs = np.where(knocked, 0.0, np.maximum(S[:, -1] - K, 0.0)) * np.exp(-r * T_days/252)

    pnls = {}
    for rebal, use_v, label in [(1,False,"Daily Δ"),(5,False,"Weekly Δ"),
                                 (1,True, "Daily Δ+V"),(5,True, "Weekly Δ+V")]:
        arr = np.zeros(n_paths)
        V0  = barrier_price(S0, K, B, T_days/252, r, sigma)
        for i in range(n_paths):
            path = S[i]; ko = False
            port = V0; dh = 0.0; vn = 0.0
            for t in range(T_days):
                if path[t] >= B: ko = True; break
                Tr = (T_days - t) / 252
                if t % rebal == 0:
                    nd  = b_delta(path[t], K, B, Tr, r, sigma)
                    dc  = nd - dh
                    port -= dc * path[t] + tc * abs(dc) * path[t]
                    dh   = nd
                    if use_v:
                        vb  = b_vega(path[t], K, B, Tr, r, sigma)
                        vv  = bs_vega(path[t], K, Tr, r, sigma)
                        nv  = -vb / vv if abs(vv) > 1e-6 else 0.0
                        pv  = bs_call(path[t], K, Tr, r, sigma)
                        vc  = nv - vn
                        port -= vc * pv + tc * abs(vc) * pv
                        vn   = nv
                port += dh * (path[t+1] - path[t])
                if use_v and vn != 0:
                    Tr2  = max((T_days - t - 1) / 252, 1e-6)
                    port += vn * (bs_call(path[t+1], K, Tr2, r, sigma)
                                  - bs_call(path[t],  K, max(Tr, 1e-6), r, sigma))
            if not ko:
                port += max(path[-1] - K, 0.0)
                port -= dh * path[-1] + tc * abs(dh) * path[-1]
            arr[i] = port * np.exp(-r * T_days / 252)
        pnls[label] = arr
    ...
    children=[
        ...
        dbc.Card(dbc.CardBody(dbc.Row([
            ...
            dbc.Col([
                ...
                dcc.Slider(id="sl-n", min=500, max=5000, step=500, value=2000,
                           marks={500: "500", 2000: "2k", 5000: "5k"},
                           tooltip={"placement": "bottom", "always_visible": False}),
            ], md=3),
            dbc.Col([
...
def update(B, sigma, n_paths, tc):
    ...
    vga    = b_vega(S0, K, B, T_ann, r, sigma)
    ...
    fig_paths.add_trace(go.Scatter(x=[None], y=[None], mode="lines",
        line=dict(color=RED, width=1.5), name=f"Knocked out ({ko_rt:.1%})"))
    fig_paths.add_trace(go.Scatter(x=[None], y=[None], mode="lines",
    ...
    fig_conv.add_trace(go.Scatter(
        ...
        y=ci_up + ci_lo[::-1],
        fill="toself", fillcolor=hex_rgba(BLUE, 0.1),
    ...
    if len(survived_payoffs) > 0:
        fig_pay.add_trace(go.Histogram(
            ...
            marker_color=GREEN, opacity=0.75, name="ITM payoffs",
        ))
    ...
    # ── Greeks vs barrier ───────────────────────────────────────────────────
    B_range = list(range(103, 148, 2))
    prices_s = [barrier_price(S0, K, b, T_ann, r, sigma) for b in B_range]
    deltas_s = [b_delta(S0, K, b, T_ann, r, sigma) for b in B_range]
